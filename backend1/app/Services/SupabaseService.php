...
class SupabaseService
{
    ...
    public function listRooms(): array
    {
        ...
        $rooms = $response->json() ?? [];
        $roomTypes = $this->listRoomTypesById();

        return array_map(static function (array $room) use ($roomTypes): array {
            $roomTypeId = (string) ($room['room_type_id'] ?? '');
            if ($roomTypeId !== '' && isset($roomTypes[$roomTypeId])) {
                $room['room_types'] = $roomTypes[$roomTypeId];
            }
    ...
    private function listRoomTypesById(): array
    {
        $response = $this->rest('GET', 'room_types', [
        ...
        return collect($types)
            ->filter(static fn (array $type): bool => isset($type['id']))
            ->keyBy(static fn (array $type): string => (string) $type['id'])
            ->all();
    }
    ...
    public function createBooking(array $payload): array
    {
        ...
        if ($response->failed() && array_key_exists('balance_due', $payload) && str_contains($response->body(), "'balance_due'")) {
            $response = $this->rest('POST', 'bookings', [], [Arr::except($payload, ['balance_due'])], [
                'Prefer' => 'return=representation',
            ]);
        }
        ...
        $booking = Arr::first($response->json() ?? []) ?? [];

        if (! array_key_exists('balance_due', $booking) && array_key_exists('balance_due', $payload)) {
            $booking['balance_due'] = $payload['balance_due'];
        }
